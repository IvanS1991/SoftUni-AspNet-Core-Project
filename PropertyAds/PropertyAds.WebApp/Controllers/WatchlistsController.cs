namespace PropertyAds.WebApp.Controllers
{
    ...
    public class WatchlistsController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            {
                Watchlists = await this.watchlistData
                    .All(this.userData.CurrentUserId())
            };
        ...
        public async Task<IActionResult> Create(WatchlistListingFormModel formModel)
        {
            await this.watchlistData
                .Create(formModel.CreatedWatchlistName);
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var viewModel = this.mapper
                .Map<WatchlistDetailsViewModel>(watchlist);

            var viewModelProperties = await this.propertyData
                .AllById(watchlist.WatchlistProperties.Select(x => x.PropertyId));
            ...
            {
                var segmentProperties = await this.propertyData
                    .All(segment.District.Id, segment.PropertyType.Id);

                viewModel.Segments
                    .Add(new WatchlistSegmentViewModel
                    {
        ...
        {
            if (await this.watchlistData
                .HasOwner(watchlistId, this.userData.CurrentUserId()) == false)
            {
                return Unauthorized();
            }

            await this.watchlistData
                .RemoveProperty(watchlistId, propertyId);
            ...
            string districtId)
        {
            ...
            await this.watchlistData
                .RemoveSegment(watchlistId, propertyTypeId, districtId);
